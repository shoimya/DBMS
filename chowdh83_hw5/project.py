...
Netid: chowdh83
PID:159958342
How long did this project take you? im too quick 


Sources:
no resources other than searching how to use filter and index fucntion etc
https://favtutor.com/blogs/list-to-tuple-python
"""
import string
...
def tokenize(query):
    if isinstance(query, list):
        return query #mainly used for parameterized inserts. 
...
class Connection(object):
    ...
    def execute(self, statement):
        ...
        if tokens[0].upper() == "CREATE" and "VIEW" not in tokens:
            ...
            return self.dataBase.createView(tokens,statement)
    ...
    def executemany(self,query,params):
        ...
        for entry in params:
            ...
            # temp = (" ".join(str(item) for item in temp)).rstrip()
            self.execute(temp)
...
class Database(object):
    ...
    def createView(self,tokens,statement):
        tokens.pop(0) #create
        tokens.pop(0) #view
        viewName = tokens.pop(0)
        tokens.pop(0) #As
        view = View()
        view.name = viewName #name of view 
        view.statement = tokens #in token form
        if "JOIN" in tokens:
            selected_headers = "".join(tokens[1:tokens.index("FROM")]).split(",")     
            # print(selected_headers)
            table1 = tokens[tokens.index("FROM")+1]   
            table2 = tokens[tokens.index("JOIN")+1]  
            view.selectedHeaders =  selected_headers
            view.tables.append(table1)
            view.tables.append(table2)
            self.views.append(view)

        else:
            if tokens.index("FROM") - 0 == 2: #selected all headers
                if tokens[1] == "*":
                    for item in self.tables:
                        if item.name == tokens[tokens.index("FROM")+1]:
                            view.selectedHeaders = item.headers
                    view.tables.append(tokens[tokens.index("FROM")+1])
                    self.views.append(view)
                else:
                    header = tokens[1]
                    tableName = tokens[tokens.index("FROM")+1]
                    if "." not in header:
                        view.selectedHeaders.append(tableName+'.'+header)
                    else:
                        view.selectedHeaders.append(header)
                    view.tables.append(tableName)
                    self.views.append(view)
            else:
                tableName = tokens[tokens.index("FROM")+1]
                selected = tokens[1:tokens.index("FROM")]
                selected = [x for x in selected if x != "," ]
                selected = [tableName+"."+x if "." not in x else x for x in selected]
                view.selectedHeaders = selected
                view.tables.append(tableName)
                self.views.append(view)
    ...
    def create(self,tokens):
        ...
        for item in headers:
            ...
            if "DEFAULT" in item:
                if item[1] == "REAL":
                    table.defaultValues.append(float(item[3])) #adding the def value
                elif item[1] == "INTEGER":
                    table.defaultValues.append(int(item[3]))
                else: #it is a string
                    table.defaultValues.append(item[3])            
            else:
                table.defaultValues.append(None)
    ...
    def insert(self, tokens):
        ...
        elif "DEFAULT" in tokens:
            r = Row()
            for i in range(len(currentTable.headers)):
                r.data[currentTable.headers[i]] = currentTable.defaultValues[i]
            currentTable.rows.append(r) #just add the default values to the rows
    ...
    def select(self, tokens):
        ...
        for item in self.views:
            if item.name == tabName:
                #we are selecting on a view
                view = item
                statementRun = view.statement
                statementRun = statementRun[:statementRun.index("BY") +1]
                if "JOIN" not in statementRun:
                    sorting = tokens[tokens.index("BY")+1:len(tokens)-1]
                    sorting = [x  for x in sorting if x != ","]
                    sorting = [view.tables[0]+"."+ x if "." not in x else x for x in sorting]

                    if len(sorting) == 1:
                        if sorting[0] in view.selectedHeaders:
                            statementRun.append(sorting[0])
                            statementRun.append(";") #the statement is complete and up to date
                            return self.select(statementRun)
                    else:
                        for i in range(len(sorting)):
                            if i != len(sorting)-1:
                                if sorting[i] in view.selectedHeaders:
                                    statementRun.append(sorting[i])
                                    statementRun.append(",")
                            else:
                                if sorting[i] in view.selectedHeaders:
                                    statementRun.append(sorting[i])
                    ...
                    ordering = tokens[tokens.index("BY")+1:len(tokens)-1]
                    ordering = [x for x in ordering if x != ","]
                    for i in range(len(ordering)):
                        if i != len(ordering)-1:
                            if view.tables[0]+"."+ordering[i] in view.selectedHeaders:
                                statementRun.append(view.tables[0])
                                statementRun.append(".")
                                statementRun.append(ordering[i])
                                statementRun.append(",")
                            elif view.tables[1]+"."+ordering[i] in view.selectedHeaders:
                                statementRun.append(view.tables[1])
                                statementRun.append(".")
                                statementRun.append(ordering[i])
                                statementRun.append(",")
                        else:
        ...
        if "DESC" in tokens:
            descending = True
            tokens = [element for element in tokens if element != "DESC"]
        ...
        if "JOIN" in tokens:
        ...
        elif "MIN" in tokens or "MAX" in tokens:
            criteria  = None
            min_ = False
            if 'MIN' in tokens:
                criteria = tokens[tokens.index("MIN") + 2]
                min_ = True
            else:
                criteria = tokens[tokens.index("MAX") + 2]

            index_criteria = currentTable.headers.index(currentTable.name+"."+criteria)
            result,sorting_criteria = self.selectHelper(tokens,currentTable)

            if min_: #do min
                return_list = []
                contain = []
                m = min(result, key=lambda x: x[index_criteria])[index_criteria]
                contain.append(m)
                tuple1 = (*contain,)
                return_list.append(tuple1)
                return return_list
            else: #do max
                return_list = []
                contain = []
                m = max(result, key=lambda x: x[index_criteria])[index_criteria]
                contain.append(m)
                tuple1 = (*contain,)
                return_list.append(tuple1)
                return return_list

        else: #there is no join and will do normal select queries 
            ...
            if filtered == False: #there was no where clause
                if descending == True:
                    result = sorted(result, key=itemgetter(*sorting_criteria), reverse=True) #sorting before selection
                else:
                ...
                if None in tokens and "NOT" not in tokens:
                    ...
                    if descending == True:
                        filtered = sorted(filtered, key=itemgetter(*sorting_criteria), reverse=True) #sorting before selection
                    else:
                        filtered = sorted(filtered, key=itemgetter(*sorting_criteria)) #sorting before selection
            ...
            if distinct: #only for single column
                ...
                if descending == True:
                    return_list = sorted(return_list, key=itemgetter(*sorting_criteria), reverse=True) #sorting before selection
                else:
                    return_list = sorted(return_list, key=itemgetter(*sorting_criteria)) #sorting before selection
...
class View(object):
    def __init__(self) -> None:
        self.name = ""
        self.type = "VIEW"
        self.statement = "" #select statement of the view
        #if the view is just like the table then, the current select will work on the view.
        self.tables = [] #tables this view selected from
    ...
    def insert_row(self,columns,list_of_values): #mainly for specified column insert
        for values in list_of_values:
            ...
            for i in range(len(self.headers)):
                row.data[self.headers[i]] = self.defaultValues[i]
